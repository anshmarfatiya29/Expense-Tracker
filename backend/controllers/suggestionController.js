  ...
  try {
    ...
    if (expenses.length === 0) {
      return res.status(200).json({
        suggestion:
          "Not enough expense data from the last 30 days to generate a suggestion. Please add more expenses.",
      });
    }
    ...
    const prompt = `
      You are a friendly and insightful financial advisor for an app called "Expense Tracker".
    ...
    res.status(500).json({
      message: "Unable to fetch budget suggestion at the moment. Please try again later.",
    });
  }
};
...
exports.getIncomeGrowthSuggestion = async (req, res) => {
  try {
    ...
    if (savings <= 0) {
      return res.status(200).json({
        suggestion:
          "Your expenses are greater than your income for the last 30 days. Focus on creating a positive saving balance before looking for investment options.",
      });
    }

    // 2. Create the AI Prompt with disclaimer
    const prompt = `
      You are a financial educator for an app called "Expense Tracker".
    ...
    res.status(500).json({
      message: "Unable to fetch income growth suggestion at the moment. Please try again later.",
    });
  }
};
